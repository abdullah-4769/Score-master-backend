...
async create(dto: CreatePlayerCapabilityDto) {
  return this.prisma.playerCapability.create({
    data: {
      ...
      badgeNames: dto.badgeNames as Prisma.JsonArray,  // cast explicitly
...
async findOne(id: number) {
  return this.prisma.playerCapability.findUnique({
    where: { id },
  })
}

  async update(id: number, dto: UpdatePlayerCapabilityDto) {
    return this.prisma.playerCapability.update({
      where: { id },
      data: dto,
    })
  }
...
async findByFormatId(FormatId: number) {
  return this.prisma.playerCapability.findMany({
    where: { FormatId },
  })
}
